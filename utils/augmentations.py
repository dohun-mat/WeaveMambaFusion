"""
Copied from https://github.com/amdegroot/ssd.pytorch/blob/master/utils/augmentations.py
"""
# -*- coding:utf-8 -*-
...
def generate_sample(sampler, image_width, image_height):
    scale = np.random.uniform(sampler.min_scale, sampler.max_scale)  # 1 or 0.3 ~ 1
    aspect_ratio = np.random.uniform(
        sampler.min_aspect_ratio, sampler.max_aspect_ratio  # 1
    )
...
def anchor_crop_image_sampling(img, bbox_labels, scale_array, img_width, img_height):
    ...
    for i, anchors in enumerate(scale_array):
        if isinstance(anchors, (list, np.ndarray)):
            for anchor in anchors:
                if abs(anchor - rand_Side) < distance:
                    distance = abs(anchor - rand_Side)
                    anchor_idx = i
        else:
            if abs(anchors - rand_Side) < distance:
                distance = abs(anchors - rand_Side)
                anchor_idx = i

    target_anchor = random.choice(scale_array[0 : min(anchor_idx + 1, 5) + 1])
    if isinstance(target_anchor, (list, np.ndarray)):
        target_anchor = random.choice(target_anchor)
    ...
    if len(sample_boxes) > 0:
        ...
        # "We keep the overlapped part of the face box if its center is in the sampled pathch."
...
def preprocess(
    ...
    remove_small_face_thresh=None,
):
    img_width, img_height = img.size
    sampled_labels = bbox_labels
    if mode == "train":
        if cfg.apply_distort:  # True
            img = distort_image(img)
        if cfg.apply_expand:  # False
        ...
        if (
            prob > cfg.data_anchor_sampling_prob and cfg.anchor_sampling
        ):  # anchor sampling prob = 50%
            scale_array = np.array(cfg.ANCHOR_SIZES)
            img = np.array(img)
            ...
            #  min_object_coverage, 1
            #  max_object_coverage, 0
            #  use_square=False): True
    ...
    if mode == "train":
        ...
        # remove small faces under 16 x 16 from target annot.
        if remove_small_faces:
            keep_index = []
            for idx, bbox_info in enumerate(sampled_labels):
                box_area = (bbox_info[3] - bbox_info[1]) * (bbox_info[4] - bbox_info[2])
                thresh_area = (remove_small_face_thresh[0] / cfg.resize_width) * (
                    remove_small_face_thresh[1] / cfg.resize_height
                )
                if math.sqrt(box_area) >= math.sqrt(thresh_area):
                    keep_index.append(idx)

            sampled_labels = [sampled_labels[idx] for idx in keep_index]
